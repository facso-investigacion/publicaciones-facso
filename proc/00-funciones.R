...
#' Separa un nombre completo en apellidos (los 2 ultimos tokens, o el unico
#' disponible si el nombre trae solo 2 palabras) e iniciales de los nombres
#' restantes. Heuristica pensada para nombres chilenos (nombre(s) + apellido
#' paterno + apellido materno); para nombres de 2 tokens degrada de forma
#' razonable a la convencion "Nombre Apellido" (anglosajona u otras).
#' La usan tanto 07-coautores.R (para calzar autores por apellido dentro de
#' una misma publicacion) como exportar-bibliometrix.R (para el campo AU).
partir_nombre <- function(nombre) {
  partes <- str_split(str_squish(nombre), " ")[[1]]
  n <- length(partes)
  if (n <= 2) {
    apellidos <- partes[n]
    nombres   <- partes[seq_len(n - 1)]
  } else {
    apellidos <- paste(partes[(n - 1):n], collapse = " ")
    nombres   <- partes[seq_len(n - 2)]
  }
  list(apellidos = apellidos, iniciales = str_sub(nombres, 1, 1))
}
